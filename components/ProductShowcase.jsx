...
export default function ProductShowcase() {
  ...
  return (
    <section id="product" className="bg-ink-50 py-20 sm:py-28">
      <div className="container-page grid gap-12 lg:grid-cols-2 lg:items-start">
        {/* Gallery */}
        <div className="space-y-4">
          <div className="aspect-square rounded-3xl overflow-hidden bg-white shadow-soft">
            <img
          ...
          <div className="grid grid-cols-4 gap-3">
            {PRODUCT.images.map((img, i) => (
              <button
                ...
                className={`aspect-square rounded-xl overflow-hidden ring-2 transition ${
        ...
        <div>
          ...
          <h2 className="h-display text-4xl sm:text-5xl font-extrabold leading-tight">
            {PRODUCT.name}
          </h2>
          <p className="text-ink-500 mt-3 text-lg">{PRODUCT.shortPitch}</p>

          <div className="mt-6 flex items-baseline gap-3">
            <span className="text-4xl font-extrabold text-ink-900">${PRODUCT.price}</span>
            <span className="text-lg text-ink-400 line-through">${PRODUCT.compareAtPrice}</span>
            <span className="chip bg-amber-100 text-amber-800">Save {discountPct}%</span>
          </div>
          <p className="text-sm text-ink-500 mt-2">or 4 interest-free payments — finalize at checkout</p>
          ...
          <div className="mt-8">
            <label className="text-sm font-semibold text-ink-700 mb-3 block">Choose your size</label>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              {PRODUCT.variants.map((v) => (
                ...
                >
                  {v.label.split(' ')[0]}
                  <span className={`block text-[10px] font-normal mt-0.5 ${variant === v.id ? 'text-white/70' : 'text-ink-400'}`}>
                    {v.label.match(/\(.+\)/)?.[0]}
                  </span>
                </button>
              ))}
          ...
          <button
            ...
            className="btn-accent w-full mt-7 text-base h-14"
          >
          ...
          <div className="mt-6 rounded-2xl bg-amber-50 border border-amber-100 px-4 py-3 text-sm text-amber-900 flex items-start gap-2">
            ...
            <span>
              <strong>Only {PRODUCT.unitsLeft} left</strong> at launch pricing — restock arrives in
              ~3 weeks at full price.
            </span>
          </div>
        </div>
      </div>
